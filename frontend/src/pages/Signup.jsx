...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:1000/api/v2/signup", Data);
    ...
    <div className="h-screen flex items-center justify-center bg-gradient-to-r from-purple-900 via-purple-950 to-black">
      <div className="relative w-96">
        ...
        <div className="relative bg-gradient-to-r from-purple-800 via-purple-900 to-black shadow-2xl rounded-2xl p-8 w-full text-center z-10 border border-purple-700">
          ...
          <Link
            to="/login"
            className="text-purple-300 text-sm hover:text-purple-400 hover:underline mt-3 transition-all duration-300 ease-in-out"
          >
            Already have an account?{" "}
            <span className="font-semibold text-purple-400">Login</span>
          </Link>
        </div>
      </div>
    </div>
